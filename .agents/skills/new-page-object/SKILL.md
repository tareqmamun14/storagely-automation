...
## Flex sections
Flex sections follow the `flex/pages/sections/*.ts` pattern and register in `index.ts` + `types.ts`; the manifest (`configs/components.ts`, `configs/facilities.ts`) is the source of truth — see [flex/AGENTS.md](../../../flex/AGENTS.md).
