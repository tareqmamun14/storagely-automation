 ...
 *   • skipping cleanly when this section was excluded by HELIX_SECTIONS
  ...
  test.describe(`Section: ${section.label}`, () => {
    test.beforeAll(() => {
      if (!isSectionEnabled(sectionId)) {
        console.log(`[helix] section "${sectionId}" excluded by HELIX_SECTIONS env var — skipping.`);
      }
    });

    for (const facility of facilities) {
      test(`${section.label} — ${facility.name}`, async ({ page }) => {
        test.skip(!isSectionEnabled(sectionId), `Section "${sectionId}" filtered out by HELIX_SECTIONS`);
...
function featureDisabled(facility: HelixFacility, sectionId: string): boolean {
