...
test.describe('Helix Editor — Exploration', () => {
  test.setTimeout(180_000);

  test('login + capture editor structure', async ({ page }) => {
    if (!HELIX_EDITOR_CREDENTIALS.password) {
      throw new Error('HELIX_PASSWORD not set — enter password in control panel and re-run.');
    }
    ensureDir(OUT_DIR);
    ...
    for (const sel of emailSelectors) {
      ...
      if (await loc.count() && await loc.isVisible().catch(() => false)) {
        await loc.fill(HELIX_EDITOR_CREDENTIALS.email);
    ...
    if (!emailFilled) {
      ...
      await first.fill(HELIX_EDITOR_CREDENTIALS.email);
    ...
    for (const sel of passwordSelectors) {
      const loc = page.locator(sel).first();
      if (await loc.count() && await loc.isVisible().catch(() => false)) {
        await loc.fill(HELIX_EDITOR_CREDENTIALS.password);
    ...
    console.log('   structure dumped to: helix/test-results/explore/05-editor-structure.json');
