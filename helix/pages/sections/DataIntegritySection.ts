  ...
  async verify(page: Page, ctx: SectionContext): Promise<SectionResult> {
    ...
    try {
      ...
      checks.push(check(
        ...
        facts.reviewCounts.length <= 1,
        facts.reviewCounts.length <= 1
          ? (facts.reviewCounts.length ? `${facts.reviewCounts[0]} reviews` : '(no review count found)')
          : `MISMATCH — page shows ${facts.reviewCounts.join(' vs ')} reviews in different spots`,
      ));

      // ── rating consistency ──
      checks.push(check(
        'star rating is consistent across the page',
        facts.ratings.length <= 1,
        facts.ratings.length <= 1
          ? (facts.ratings.length ? `${facts.ratings[0]} ★` : '(no rating found)')
          : `MISMATCH — page shows ${facts.ratings.join(' vs ')} ★ in different spots`,
      ));
