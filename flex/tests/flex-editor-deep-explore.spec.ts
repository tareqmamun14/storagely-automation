...
test.describe('Flex Editor — Deep Exploration', () => {
  ...
  test('login, explore palette, capture component list + preview iframe', async ({ page }) => {
    ...
    await page.locator('input[type="email"]').fill(FLEX_EDITOR_CREDENTIALS.email);
    await page.locator('input[type="password"]').fill(FLEX_EDITOR_CREDENTIALS.password);
