 ...
 *
 * Run: npx playwright test --config=flex/playwright.config.ts --project=discovery
 *      flex/tests/editor/_discovery/modern-unit-card.spec.ts
 */
...
test('discovery: Modern Unit Card properties (assumes MUC pre-placed on /test)', async ({ page }) => {
  ...
  const editor = new FlexEditorPage(page);

  // 1. Open editor on /test page (MUC already placed by user)
  await editor.goto(FLEX_EDITOR_URLS.editor);
  ...
  //    Clicking the button in the editor preview triggers Flex's section selection.
  ...
  //    force: true because Flex's outer overlay (div.absolute.inset-0.z-10)
