...
test.describe('Flex Editor — Watch mode', () => {
  ...
  test('go directly to editor URL, sign in, keep browser open', async ({ page }) => {
    ...
    await page.locator('input[type="email"]').first().fill(FLEX_EDITOR_CREDENTIALS.email);
    await page.locator('input[type="password"]').first().fill(FLEX_EDITOR_CREDENTIALS.password);
