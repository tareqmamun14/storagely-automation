 ...
 *
 * Captures element-scoped screenshots of key visual areas on each Flex
 ...
 *   Commit baselines to git:      → `git add flex/tests/live/visual/baseline.spec.ts-snapshots/`
...
async function getVisibleHeroLocator(page: Page) {
  const tagged = await page.evaluate((vw) => {
    ...
    document.querySelectorAll('[data-flex-hero]').forEach(el => el.removeAttribute('data-flex-hero'));
    ...
    imgs[0].setAttribute('data-flex-hero', 'true');
    return true;
  ...
  return page.locator('img[data-flex-hero="true"]');
}
